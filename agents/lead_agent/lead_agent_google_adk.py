# agents/lead_agent/lead_agent_google_adk.py
...
class LeadAgentGoogleADK:
    def __init__(self, model_name=None, use_openai_model=False):
        ...
        if use_openai_model:
            ...
            print(f"Google ADK Agent configured to attempt using OpenAI model: {model_name or settings.DEFAULT_OPENAI_MODEL} via LiteLLM")
        else:
        ...
        self.agent = Agent(
            ...
            tools=[
                web_prospector_adk_tool,
            ],
        )
        print("LeadAgentGoogleADK initialized.")

    def process_lead_request(self, task_description: str, user_id="user123", session_id=None):
